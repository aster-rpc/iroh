...
| `iroh-blobs` | `v0.103.0` | `aster-iroh-blobs-v0.103.0-p1` | `a2e15663c4015dc37118845d160100ebdbdb4aeb` | `upgrade/iroh-blobs-v0.103` |
...
- iroh stack carried: noq fork `[patch.crates-io]` entries (pinned to
  ...
  fmt fixups.
- `iroh-blobs` gained one post-tag commit on 2026-08-13 (functional patch row 8
  below): downloader progress observability, retagged
  `aster-iroh-blobs-v0.103.0-p1`. Canonical source branch is
  `fix/downloader-progress-observability`, based on `v0.103.0`.
...
git -C /Users/emrul/dev/aster/iroh-blobs  switch --detach aster-iroh-blobs-v0.103.0-p1
...
| 8 | `iroh-blobs` | `a2e15663` (on `upgrade/iroh-blobs-v0.103`) / `adab99f1` (fix branch) | `src/api/downloader.rs`, `src/util.rs` | Makes download progress observable: root fetch in `Split` mode reports through the real sink instead of `Drain`; the split fan-in is drained after the futures stream ends (previously a fast final child lost its `PartComplete`); `TryProvider` moves after the `local.is_complete()` early return so an already-resident request is not reported as an attempt; and a new additive `DownloadProgressItem::BytesTransferred(u64)` gives an exact per-attempt payload total (`Progress` is a cumulative offset that includes local bytes and is re-based per child, so it cannot be summed). Provider ordering, failover, resumption and split scheduling are unchanged. | `aster_transport_core::CoreBlobsClient::run_download` folds this stream into `CoreFetchReport`, which `aster::Blobs::download_hash_from` / `download_hash_to_store_from` / `download_collection_from` expose to portal-sync for topology-aware provider ranking. | Medium conflict risk in `execute_get` and `handle_download_split_impl`. Canonical source is `fix/downloader-progress-observability` (based on `v0.103.0`). Preserve: `pool.get_or_connect` stays ahead of the local scan so the dial and bitfield read still overlap; `BytesTransferred` comes from the returned `Stats::payload_bytes_read` on success and from the last raw sink value on failure. Four tests in `api::downloader::tests` pin each behaviour. |
